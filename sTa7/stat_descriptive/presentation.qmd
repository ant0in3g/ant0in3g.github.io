...
::: {.callout-note}
Soit un ensemnle de $p$ famille. L'effectif total $N$ est donné par 
$$N= \sum_i^p n_i$$
où $n_i$ est l'effectif de la famille $i$.
:::
...
Soit la fonction $$F : \mathbb{R} \to [0,1]$$
$F(x)$ : pourcentage d'individus dont la valeur du caractère $\leq x$.\par
...
Avec la fonction de répartition précedemment présentée :
$$F(M_e^-) < 0.5 < F(M_e^+)$$
...
::: {.callout-important}
**La moyenne** quant à elle est la valeur de $p$ qui permet de minimiser 
$$\sum_{i=1}^{19} \left(p-x_i\right)^2$$
:::
