...
Son équation polaire est $$\rho = 1 +\cos\left(n\theta\right) + \sin^2\left(n\theta\right)$$
...
Le domaine de définition est $$D_1 = \mathbb{R}$$
On a 
...
on peut alors affirmer que $\rho$ est périodique de période $\dfrac{\pi}{2}$. On peut limiter l'étude à un intervalle d'amplitude $\dfrac{\pi}{2}$. On completera par autant de rotations qu'il faut pour retomber sur l'arc de courbe initial.

$$D_2 = \left[-\dfrac{\pi}{4},\dfrac{\pi}{4}\right]$$
De plus on remarque que $\rho$ est paire en $\theta$, c'est à dire 
$$\rho\left(-\theta\right)=\rho\left(\theta\right)$$
Donc les point $M_1\left(\rho,\theta\right)$ et $M_2\left(\rho,-\theta\right)$ sont symétriques par rapport à l'axe des abscisses. On peut donc limiter l'étude à $$D_3 = \left[0,\dfrac{\pi}{4}\right]$$
